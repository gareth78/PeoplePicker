...
const CACHE_KEY = 'lists:locations';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    do {
      ...
      data.forEach((user) => {
        const location = buildLocation(user.profile?.city, user.profile?.countryCode);
        if (location) {
    ...
    const items = Array.from(locations).sort((a, b) => a.localeCompare(b));
    ...
    return badGateway('Failed to load locations');
  }
}
...
function buildPath(cursor: string) {
  ...
  params.set('fields', 'profile.city,profile.countryCode');
